...
//	}
	public void Ticket(Booking booking) {
		...
		String bodymsg = "Hi "+booking.getUsername()+"\n"+"Thanks for choosing "+booking.getMultiplexName()+"\nYour booking is Successfull with booking Id"+booking.getId()+"with Seat No's"+booking.getBookedSeatNos()+"for the movie "
				+booking.getMovieName()+"in "+booking.getScreenName()+"for the show at "+booking.getShowtime()+"\nEnjoy the Show😎 \nFor more multiplexdetails"+booking.getMultiplexAddress();
	 details1.setMsgBody(bodymsg);
	 details1.setSubject("Movie Ticket Booking Confirmation");
	 details1.setRecipient(booking.getUserEmail());
